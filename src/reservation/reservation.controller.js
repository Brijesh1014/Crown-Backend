...
const createReservation = async (req, res) => {
  try {
    const { restaurantId, customerName,customerEmail,customerPhoneNo, reservationDate,comments, startTime, capacity } =
      req.body;
    ...
    if (totalCapacity < capacity) {
      return res
        .status(400)
        .json({ success: false, message: "No suitable tables available,Please contact to our restaurant manager" });
    }
  ...
  try {
    ...
    if (allAvailableSlots.length === 0) {
      return res.status(404).json({
        success: false,
        message: "No suitable tables available,Please contact to our restaurant manager",
      });
    }
